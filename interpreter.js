...
Interpreter.prototype.getValueFromScope = function ( name ) {
  ...
  while ( scope ) {
    ...
    let delegate = scope.properties.delegate;
    if ( delegate && delegate.hasProperty( nameStr ) ) {
      return delegate.getProperty( nameStr );
    }
...
Interpreter.prototype.getMethodFromDelegate = function ( name ) {
  var scope = this.getScope();
  var nameStr = name.toString();
  while ( scope ) {
    var delegate = scope.properties.delegate;
    if ( delegate && delegate.hasMethod( nameStr ) ) {
      var m = delegate.getMethod( nameStr );
      if ( m ) {
        return m;
      }
    }
...
Interpreter.prototype.delegateHasProperty = function ( name ) {
  var scope = this.getScope();
  var nameStr = name.toString();
  while ( scope ) {
    var delegate = scope.properties.delegate;
...
Interpreter.prototype.setValueToScope = function ( name, value ) {
  ...
  while ( scope ) {
    ...
    let delegate = scope.properties.delegate;
    if ( delegate && delegate.hasProperty( nameStr ) ) {
      return delegate.setProperty( nameStr, value );
    }
...
Interpreter.prototype.populateScope_ = function ( node, scope ) {
  ...
  function recurse( child ) {
    if ( child.constructor == thisInterpreter.ast.constructor ) {
      thisInterpreter.populateScope_( child, scope );
    }
  }
  ...
  if ( !state.doneCallee_ ) {
    ...
    if ( node.arguments[n] ) {
      ...
      if ( state.func_.node ) {
        ...
        let func_ = state.func_;
        let node = func_.node;
        let scope = this.createScope( node.body, func_.parentScope );

        var delegate = node.__delegate;
        if ( delegate ) {
          this.setProperty( scope, 'delegate', delegate );
        }

        // Add all arguments.
        let args = state.arguments;
        for ( let i = 0; i < node.params.length; i++ ) {
          var paramName = this.createPrimitive( node.params[i].name );
          var paramValue = args.length > i ? args[i] : this.UNDEFINED;
          this.setProperty( scope, paramName, paramValue );
        }

        if ( node.params.length === 0 ) {
          var value = args.length > 0 ? args[0] : this.UNDEFINED;
          this.setProperty( scope, 'it', value );
        }
        ...
        let name = state.func_;
        let this_ = state.funcThis_;
        let args = state.arguments;
        let invoker = this.getMethodFromDelegate( name );
        if ( invoker ) {
          state.value = invoker.nativeFunc.apply( this_, args );
          return;
        }
        //try invoking a property setter
        if ( this.delegateHasProperty( name ) ) {
          this.setValueToScope( name, args );
          return;
        }

        // Are we invoking a property setter
